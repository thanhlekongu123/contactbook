...
class ContactService {
    ...
    async findById(id) {
        return await this.contact.where('id', id).select('*').first();
        
    }

    async update(id, payload) {
        const update = this.#getContact(payload);
        return await this.contact.where('id', id).update(update);    
    }
    ...
    async allFavourite() {
        return await this.contact.where('favourite', 1).select('*');
    }
